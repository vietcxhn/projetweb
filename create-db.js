...
db.prepare('CREATE TABLE user (question_id INTEGER PRIMARY KEY AUTOINCREMENT, set TEXT, answer INTEGER, answered INTEGER DEFAULT 0, answered_by TEXT)').run();
db.prepare('DROP TABLE IF EXISTS set_of_questions').run();
db.prepare('CREATE TABLE user (set_of_question_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)').run();
